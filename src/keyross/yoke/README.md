...
- **Subagents**: a Deep Agents subagent (the `task` tool) does not inherit the main agent's middleware, so the yoke measures what a delegated run brings back — the documents it returns, and those it wrote on a shared disk — and keeps a red one out of the agent's files; the main agent gets the red flag.
...
- **A plain LangChain agent** (`langchain.agents.create_agent`, no Deep Agents) whose own tools write to disk: `Yoke(gauge=..., backend=LocalFiles("out"), write_tools=("save_invoice",))` — `LocalFiles` maps the tool's paths into one folder and refuses any path outside it.
...
Try it offline — a scripted model writes an invoice with a wrong total, with and without the yoke: `python examples/deepagents/invoice_agent.py`. Measure it on a real model: [bench/einvoice](../../../bench/einvoice/README.md).
